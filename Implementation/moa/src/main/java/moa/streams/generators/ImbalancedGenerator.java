...
//        	TODO: Restore to original code.
        	boolean result = intersectingEuclideanDistanceFrom(otherCentre, this.centre, this.radiuses) + intersectingEuclideanDistanceFrom(this.centre, otherCentre, otherRadiuses) + borderlineRadius + otherBorderlineRadius > euclideanDistanceBetween(this.centre, otherCentre);
//        	if (!result) System.out.println("Fail!");
    ...
    protected static class Distribution implements Serializable {
        ...
        private double sumOfRatio() {
        	return this.safeRatio + this.borderlineRatio + this.rareRatio + this.outlierRatio;
        }
    }
    ...
    protected int currentPositiveType;
    ...
    public int getCurrentPositiveType() {
    	return this.currentPositiveType;
    }
    
    public double getCurrentBorderlinePortion() {
    	return this.distribution.borderlineRatio / this.distribution.sumOfRatio();
    }
    
    public double getCurrentRarePortion() {
    	return this.distribution.rareRatio / this.distribution.sumOfRatio();
    }
    
    public double getCurrentOutlierPortion() {
    	return this.distribution.outlierRatio / this.distribution.sumOfRatio();
    }
    
    public Random getInstanceRandom() {
    	return this.instanceRandom;
    }
    ...
    protected InstanceExample nextNegativeInstance() {
        ...
        do {
        	for (int i = 0; i < numAtts; i++) {
//        		TODO: attVals[i] = (this.instanceRandom.nextDouble() * 2.0) - 1.0;
        		attVals[i] = (this.instanceRandom.nextDouble() * 4.0) - 2.0;
            }
    ...
    protected double[] nextOutlierPoint() {
    	...
    	do {
        	for (int i = 0; i < numAtts; i++) {
//        		TODO: attrs[i] = (this.instanceRandom.nextDouble() * 2.0) - 1.0;
        		attrs[i] = (this.instanceRandom.nextDouble() * 4.0) - 2.0;
            }    		
...
//        Random modelRand = new Random(this.modelRandomSeedOption.getValue());
        ...
        for (int i = 0; i < distribution.centroids.length; i++) {
            ...
            for (int j = 0; j < randRadiuses.length; j++) {
                randRadiuses[j] = radiusCap / 2 + (this.modelRandom.nextDouble() * radiusCap / 2);
                radiusesSum += randRadiuses[j];
            }
            double randBorderlineRadius = (0.3 + this.modelRandom.nextDouble() * 0.3) * radiusesSum / randRadiuses.length;
...
//	            	TODO: randCentre[j] = this.modelRandom.nextDouble() - 0.5;
	                randCentre[j] = 2.0*this.modelRandom.nextDouble() - 1.0;
	            }
	            foundPlacement = true;
	            for(int j = 0; j < i; j++) {
