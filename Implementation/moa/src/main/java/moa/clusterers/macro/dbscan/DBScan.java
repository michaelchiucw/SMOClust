...
public class DBScan extends AbstractMacroClusterer {
	...
	public Clustering getClustering(Clustering microClusters) {
		if (microClusters != null && microClusters.size() != 0) {
			...
			// c.setId(i++);
			// }
//			System.out.println("# marco: " + result.size());
//			AutoExpandVector<Cluster> clusts = result.getClusteringCopy();
//			for (int i = 0; i < clusts.size(); ++i) {
//				System.out.println("Centre of cluster " + i + ": " + Arrays.toString(clusts.get(i).getCenter()));
//				System.out.println("Weight of cluster " + i + ": " +clusts.get(i).getWeight());
//			}
			return result;
		}
