...
public abstract class CFCluster extends SphereCluster {
	...
	protected double N;
	/**
	 * Number of points in the cluster that did not record time stamp.
	 */
	protected double unrecordedN;
	/**
	...
	public double[] SS;
	/**
	 * Linear sum of time stamps of all the points added to the cluster.
	 */
	protected double LS_timestamp;
	/**
	 * Squared sum of time stamps of all the points added to the cluster.
	 */
	protected double SS_timestamp;
	 ...
	 * this kernel.
	 */
	public CFCluster(Instance instance, int dimensions, long timestamp, boolean isRecordTimeStamp) {
		this(instance.toDoubleArray(), dimensions, timestamp, isRecordTimeStamp);
	}
	...
	public CFCluster(double [] center, int dimensions, long timestamp, boolean isRecordTimeStamp) {
		this.N = 1;
		this.LS = center;
		this.SS = new double[dimensions];
		...
		this.unrecordedN = isRecordTimeStamp ? 0 : 1;
		this.LS_timestamp = isRecordTimeStamp ? timestamp : 0;
		this.SS_timestamp = isRecordTimeStamp ? timestamp * timestamp : 0;
	}
	...
	public CFCluster(CFCluster cluster) {
		...
		this.unrecordedN = cluster.unrecordedN;
		this.LS_timestamp = cluster.LS_timestamp;
		this.SS_timestamp = cluster.SS_timestamp;
	}
	
	public void insert(Instance instance, long timestamp, boolean isRecordTimeStamp) {
		this.N++;
		...
		if (isRecordTimeStamp) {
			this.LS_timestamp += timestamp;
			this.SS_timestamp += timestamp * timestamp;
		} else {
			this.unrecordedN++;
		}
	}
	...
	public double getMeanTimeStamp() {
		return this.N - this.unrecordedN <= 0 ? 0 : this.LS_timestamp / (this.N - this.unrecordedN);
	}
	
	public double getTotalTimeStamp() {
		return this.LS_timestamp;
	}
	
	public double getSDTimeStamp() {
		return this.N - this.unrecordedN <= 0 ? 0 : Math.sqrt(this.SS_timestamp / (this.N - this.unrecordedN) - this.getMeanTimeStamp() * this.getMeanTimeStamp());
	}
