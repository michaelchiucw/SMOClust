...
//                        System.out.println("Overlap C"+(clustering.size()+c)+" ->C"+(clustering.size()+c1)+": "+overlap);
    ...
    public int getMaxInclusionCluster(Instance point) {
    	int maxInclusionClusterIndex = 0;
    	double maxInclusion = 0.0;
        for (int i = 0; i < clusters.size(); i++) {
        	double curretnInclusion = clusters.get(i).getInclusionProbability(point);
            if (curretnInclusion >= maxInclusion) {
            	maxInclusion = curretnInclusion;
            	maxInclusionClusterIndex = i;
            }
        }
        if (maxInclusion == 0.0) {
        	return -1;
        }
        return maxInclusionClusterIndex;
    }
